...
pub(crate) fn phase2_all(pre: &mut [CgPre]) -> Phase2Out {
...
Phase2Out { nodes, calls: edges, unresolved, deferred, n_calls, n_builtin_calls }
}

pub(crate) fn stitch_all(deferred: &[Value], index: &HashMap<String, usize>, pre: &[CgPre]) -> (Vec<Value>, Vec<Value>, usize) {
    let mut edges: Vec<Value> = Vec::new();
    let mut unresolved: Vec<Value> = Vec::new();
...
pub fn callgraph_dir(root: &Path, max_files: usize) -> Value {
    ...
    let Phase2Out { mut nodes, calls: mut edges, mut unresolved, deferred,
                    n_calls, n_builtin_calls } = phase2_all(&mut pre);
    ...
    let (st_edges, st_unresolved, n_stitched) = stitch_all(&deferred, &index, &pre);
        edges.extend(st_edges);
        unresolved.extend(st_unresolved);
    // 稳定排序 + 统计
