//! sha256 —— 手写 SHA-256（FIPS 180-4），零第三方 crate 红线下的唯一选择。
//! 用途：app_audit 的 asar integrity 自标定/逐条目校验（S85）。
//! 对齐目标：Python hashlib.sha256().hexdigest()——小写 hex，与实现无关，
//! 单元测试用 NIST 标准向量 + 长输入钉死。
...
/// 返回小写 hex（64 字符），语义等价 hashlib.sha256(data).hexdigest()。
