...
fn op_sys(argv: &[String]) -> (i32, String) {
...
fn op_fs(argv: &[String]) -> (i32, String) {
    let op = argv.first().map(|s| s.as_str()).unwrap_or("");
...
fn op_taint(argv: &[String]) -> (i32, String) {
...
fn op_scan(argv: &[String]) -> (i32, String) {
    let op = argv.first().map(|s| s.as_str()).unwrap_or("");
    let path = argv.get(1).map(|s| s.as_str()).unwrap_or("");
    if path.is_empty() {
...
fn op_search(argv: &[String]) -> (i32, String) {
...
fn op_semantic(argv: &[String]) -> (i32, String) {
    let root = argv.first().map(|s| s.as_str()).unwrap_or("");
    let query = argv.get(1).map(|s| s.as_str()).unwrap_or("");
...
fn run_op(domain: &str, argv: &[String]) -> (i32, String) {
    match domain {
        ...
        "taint" => op_taint(argv),
        "scan" => op_scan(argv),
        "search" => op_search(argv),
        "semantic" => op_semantic(argv),
