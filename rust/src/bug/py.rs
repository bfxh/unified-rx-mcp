...
pub(crate) fn collect_defs(n: &PyNode, defined: &mut HashSet<String>, imported: &mut Vec<(String, usize)>) {
    match n.kind {
        ...
        "Import" => {
            for a in &n.children {
                ...
                upsert_import(imported, key, n.line);
            }
        }
        "ImportFrom" => {
...
pub(crate) fn call_rules(n: &PyNode, f: &PyNode, path: &str, lines: &[&str], issues: &mut Vec<Issue>) {
...
pub(crate) fn scan_python(src: &str, path: &str) -> Vec<Issue> {
    ...
    while let Some(n) = q.pop_front() {
        ...
        collect_defs(n, &mut defined, &mut imported);
    }
    for (k, _) in &imported {
    ...
    while let Some(n) = q.pop_front() {
        ...
        if n.kind == "Call"
            && let Some(f) = n.children.first() {
                call_rules(n, f, path, &lines, &mut issues);
            }
