...
impl Plugin for InteractPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<Picked>().add_systems(
            ...
            (left_click, putback, preview_follow, rotate_picked).chain(),
        );
    }
}
    ...
    if let Some(pm) = picked.module.clone() {
        ...
        match asm.0.place(&pm.def_id, cell, pm.rotation) {
            Ok(_) => {
                ...
                let center = crate::render_bridge::module_transform(cell, rot, dims).translation;
...
fn preview_follow(
    ...
    if let Ok((e, mut tf)) = preview_q.single_mut() {
        ...
        let mat = materials.add(StandardMaterial {
            base_color: Color::srgba(0.5, 0.8, 1.0, 0.45),
...
mod tests {
    ...
    fn click(app: &mut App, pos: Vec2) {
        let mut wq = SystemState::<Query<Entity>>::new(app.world_mut());
        let win_entity = wq.get(app.world()).iter().find(|e| {
