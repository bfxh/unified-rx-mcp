...
pub fn resolve_cell(
    ...
    if rot >= 24 { return cell; }
    // 漏洞A修复：outward 为 0 时防死循环（默认外移 +X）
    let outward = if outward == Vec3i(0, 0, 0) { Vec3i(1, 0, 0) } else { outward };
    let Some(def) = asm.defs.get(def_id) else { return cell };
    let occ: std::collections::HashSet<Vec3i> = asm.modules.values().flat_map(|md| md.cells.iter().copied()).collect();
...
/// 尝试放置（严格：必须连接）
pub fn try_place(asm: &mut Assembly, def_id: &str, cell: Vec3i, rot: u8) -> Result<u32, PlaceError> {
...
mod tests {
    ...
    fn def() -> ModuleDef {
        ModuleDef {
            id: "nexus.b".into(), name: "b".into(), corp: "nexus".into(),
