...
use crate::{
    ActiveVehicle, AsmRes, CursorPos, HudState, PLACING_NEW, Picked, Selection, VehicleMenu,
    cat_color,
};
...
pub fn ui_category_list_system(
    ...
    *last = Some(*mode);
    let font = font_cache
        .get_or_insert_with(|| asset_server.load("fonts/simhei.ttf"))
        .clone();
...
fn spawn_category_slots(children: &mut EntityCommands, font: &Handle<Font>, asm: &AsmRes) {
    ...
    cats.extend(
        ...
        .into_iter()
        .filter(|c| asm.0.defs.values().any(|d| d.category == *c))
        .map(Some),
    );
...
pub fn ui_corp_system(
    ...
    for (slot, interaction) in slots.iter_mut() {
        ...
        filter.0 = if slot.corp.is_empty() {
            None
        } else {
            Some(slot.corp.clone())
        };
    }
}
...
pub fn ui_inventory_system(
    ...
    for (slot, mut border) in slots.iter_mut() {
        border.set_all(if slot.def_id == sel.def_id {
            SLOT_ACTIVE_BORDER
        } else {
            SLOT_BORDER
        });
    ...
    *last = Some(current.clone());
    ...
    let mut defs: Vec<&vxl_core::module::ModuleDef> = asm
        .0
        .defs
        .values()
    ...
    let Ok((entity, _)) = content.single_mut() else {
        return;
    };
    commands.entity(entity).despawn_children();

    let rows: Vec<Vec<&vxl_core::module::ModuleDef>> = defs
...
pub fn ui_item_scroll_system(
    ...
    if cursor.pos.is_none_or(|p| p.x > WHEEL_PANEL_BOUNDARY) {
        return;
    }
...
pub fn ui_tooltip_system(
    ...
    let Some(def) = asm.0.defs.get(&def_id) else {
        return;
    };
...
pub fn ui_status_system(
    ...
    let name = if hud.selected.is_empty() {
        ...
        asm.0
            ...
            .get(&hud.selected)
            .map(|d| d.name.clone())
            .unwrap_or_else(|| hud.selected.clone())
    };
    for mut t in texts.p0().iter_mut() {
        t.0 = format!("选中: {name}");
    }
    let status = if hud.status.is_empty() {
        "空闲".to_string()
    } else {
        hud.status.clone()
    };
...
pub fn ui_vehicle_system(asm: Res<AsmRes>, mut texts: Query<&mut Text, With<VehicleInfoText>>) {
    let a = &asm.0;
    ...
    for md in a.modules.values().filter(|m| m.vehicle == vehicle) {
        if let Some(def) = a.defs.get(&md.def_id) {
            ...
            if def.category == Category::Wheel {
                wheels += 1;
            }
        }
        count += 1;
    }
    let hp = if hp_max > 0.0 {
        hp_sum / hp_max * 100.0
    } else {
        0.0
    };
...
pub fn ui_toast_system(
    ...
    let Some((text, mut left)) = toast.0.take() else {
        return;
    };
        ...
        .filter(|m| {
            m.vehicle == vehicle
                && asm
                    ...
                    .defs
                    .get(&m.def_id)
                    .is_some_and(|d| d.category == Category::Wheel)
        })
        .count();
