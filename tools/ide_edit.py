...
S93 起 locate_edit / code_context / ide_rename 三件转调 rx-ide.exe
（唯一实现在 rust/src/ide.rs）：遍历（os.walk 3.14 junction 语义 + 13 跳过
目录）、忽略大小写命中、limit*3 双层停机、references_in_scan 计数口径、
RAW split 保留 \r、radius 钳制、10MB getsize 门（在沙盒 resolve 之前——
沙盒外文件报"文件不可读"的旧漏斗原样保留）、空符号 count("")=len+1 怪癖
全部在 exe 侧等价复刻；语义等价由 S93 对照实验证明（51 场景 masked 全等）。
ide_edit_multi / ide_batch_edit 是结构化写面，留在 Python（ast 语法门 +
LSP 泵不可原生平替）。
"""
import ast
import os
...
def locate_edit(path, query, max_files=100, limit=10):
    return _rx_ide_call(["locate", path, query,
                         str(int(max_files)), str(int(limit))])
...
def code_context(path, cursor_line=0, radius=30):
    return _rx_ide_call(["context", path,
                         str(int(cursor_line)), str(int(radius))])
...
def ide_rename(root, symbol, new_name, include_plan=False):
    return _rx_ide_call(["rename", root, symbol, new_name,
