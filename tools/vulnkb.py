...
KB = [
    ...
    {"id": "kb-eval-exec", "rules": ["eval_exec", "eval_call", "new_function"],
     ...
     "tags": ["注入", "CWE-95", "RCE"]},
    {"id": "kb-shell-inject", "rules": ["child_process", "py_shell_true"],
     ...
     "tags": ["注入", "CWE-78"]},
    {"id": "kb-sql-inject", "rules": ["sql_concat"],
     "langs": ["python", "javascript", "java"], "title": "SQL 拼接注入（静态拼接模式层，S131）",
    ...
    {"id": "kb-path-traversal", "rules": [],
     ...
     "tags": ["路径", "CWE-22", "未覆盖"]},
    # ---------- Python 逻辑面 ----------
    {"id": "kb-bare-except", "rules": ["bare_except", "except_pass"],
    ...
    {"id": "kb-resource-leak", "rules": [], "langs": ["any"], "title": "资源泄漏（本仓未覆盖）",
     ...
     "tags": ["资源", "未覆盖"]},
    # ---------- S131（P2-B）新增覆盖：反序列化 / 弱哈希 / mktemp / zip-slip ----------
    {"id": "kb-deserialization", "rules": ["pickle_loads", "yaml_unsafe_load"],
     "langs": ["python"], "title": "不可信数据反序列化（pickle/yaml）",
     "cause": "pickle.loads/yaml.load(无 Loader) 反序列化外部数据可构造任意对象→代码执行",
     "fix": "改用 json/显式 schema 解析；yaml 固定 yaml.safe_load（或显式 Loader=SafeLoader）；必用 pickle 时只反序列化自己签名过的数据",
     "precedent": "本仓 rust_taint_scan 的 EXEC_SINKS 含 pickle.loads/yaml.load（污点面互证）；S131 静态模式规则与之互补（无需数据流即可命中裸用法）",
     "tags": ["注入", "CWE-502", "反序列化"]},
    {"id": "kb-weak-hash", "rules": ["weak_hash_password"],
     "langs": ["python"], "title": "弱哈希用于口令场景（md5/sha1）",
     "cause": "md5/sha1 碰撞与暴力成本低，用于口令存储等于明文慢性泄露",
     "fix": "口令用 argon2/bcrypt/scrypt（带盐慢哈希）；校验和场景可继续用 md5（规则按口令语境收窄）",
     "precedent": "本仓 secrets_hunt 同样口径：先命中再人工确认上下文（S131 规则只在同行含口令词时报）",
     "tags": ["加密", "CWE-916"]},
    {"id": "kb-mktemp", "rules": ["mktemp_race"],
     "langs": ["python"], "title": "tempfile.mktemp 名字-创建竞态（TOCTOU）",
     "cause": "mktemp 只返回名字不创建文件，攻击者可在两步之间抢占该路径（符号链接/覆盖）",
     "fix": "用 mkstemp/NamedTemporaryFile/TemporaryDirectory（创建与命名原子）",
     "precedent": "与 kb-toctou 同族；本仓沙盒 resolve 用原子校验而非两段式判断（同思路）",
     "tags": ["竞态", "CWE-377"]},
    {"id": "kb-zip-slip", "rules": ["zip_extractall"],
     "langs": ["python"], "title": "解压 zip-slip（extractall 成员路径未校验）",
     "cause": "压缩包成员名含 ../ 或绝对路径时 extractall 会写出目标目录之外",
     "fix": "逐成员校验 realpath 在目标根内（或用 3.12+ 的 data filter）",
     "precedent": "本仓 app_clean/strictly_under 的包含性校验同款思路（S125 junction 修复）",
     "tags": ["路径", "CWE-22", "zip-slip"]},
