...
**当前 v2.70.0（S154）**：80 工具 / 14 域（core 档 54 件）；**测量口径修正 + taint 去克隆**——
①**诚实 A/B 复测**（S150 备份旧 exe vs 当前，交错 5 轮）：bug_scan **3.05×**、
secrets_hunt **3.07×**、rust_taint_scan **1.10×**——S152/S153 记账的绝对值含机器
负载噪声，**比值以交错 A/B 为准**（教训：基线不能跨时段比绝对值）；
②taint 传播热循环去克隆（`textual`/`base` 改借用），输出逐字节一致，再贡献 ~5%；
③**剩余 280ms 唯一瓶颈已锁定=传播算法结构**（4 轮不动点 × 全量重扫；`callgraph_targets`
已排除嫌疑）——下轮按"反向索引 × 脏集合"做增量传播。
