...
**当前 v2.76.0（S160）**：82 工具 / 14 域（core 档 54 件）；**CI 审核再加两道硬门**——
...
|---|---|---|
| 工具面 | 183（注入面 200+） | **82 个组合工具 / 14 域**（core 档 54 件） |
...
| 🕵️ attack (8) | `input_fuzz` `path_probe` `big_input` `rust_taint_scan` `auth_gate_sweep` `attack_cruise`（S133：全攻击面一键巡航）`cargo_audit`（Rust 依赖安全审计：RustSec 薄壳，需授权） — 自攻面常驻 |
