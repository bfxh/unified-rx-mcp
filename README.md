...
**当前 v2.67.0（S151）**：80 工具 / 14 域（core 档 54 件）；**常驻服务加速（不变质量）**——
用户指令：「命令行需要不变质量的情况下加速」。三条线一起上：
①**构建层**（S150）：LTO+单编译单元+去符号，启动中位 24.41→17.21ms（−29%）、
2.89MB→364KB；本轮再验 `opt-level` 2/3/s **无可测差异**（负结果入档，保持 2）。
②**运行层（本轮主菜）**：`rx-svc.exe serve` **stdio 常驻**——宿主起一次，请求/应答
走管道（无端口无令牌，宿主退出即自退）；覆盖 sys/scan/search/semantic/taint
五域，**实测 std_check 5.53ms → 0.25ms（22.5×）**；服务与 CLI 输出**逐字节一致**
（测试锁），不可用时自动回退按次 spawn（行为与报错语义不变）。首版 TCP 环回被
本机安全栈间歇拦截（min 0.4ms/中位 15ms）→ 弃用；`UNIFIED_RX_SVC=off` 可关。
③**门禁**：cli-bench（金标准 9 条 + 计时）、性能配置锁、"服务不得比 CLI 慢 2×"
回归锁——加速是承诺，不是一次性动作。
