...
```powershell
pwsh -File scripts/vuln-scan.ps1 -ProjectPath <项目> -Category all
```
...
| 文档 | 内容 |
|---|---|
| `references/architecture-principles.md` | 架构原则（依赖倒置/无环/稳定依赖/稳定抽象/接口隔离） |
| `references/coding-conventions.md` | 编码规范（命名/注释/错误处理/内存/文件组织） |
| `references/collaboration-workflow.md` | 架构师-程序员双智能体协作 |
| `references/quality-metrics.md` | 质量度量公式与评分解读 |
| `references/regression-guard.md` | 回归防护与演进质量 |
| `references/ai-patterns-catalog.md` | AI 味模式目录（TEXT-001~008 / DEAD-001~010） |
...
> scripts/mcp_server.py：兼容保留（旧版 MCP 桥——v3.1 起 MCP 职责剥离，
> 脚本保留仅供旧环境回退；新环境用独立 MCP 配置层，不再维护此桥）。
>
