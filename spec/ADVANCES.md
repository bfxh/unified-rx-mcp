...
- **落地（S105 实装）**：`tools/aci.py` 输出纪律层 + registry 接线——
  ①**空结果显式说明**：9 个工具（code_search/code_semantic/locate_edit/
  ide_outline/bug_scan/ast_scan/fs_list/repo_map/dep_graph）空结果时加 `hint`
  给下一步（bug_scan/ast_scan 的提示明确指向 VULN-HUNTING 附录 B 的覆盖边界，
  防"无发现=无漏洞"误读）；②**截断提示**：顶层列表截断提示 cursor 续读、
  字符串截断提示分段读取、嵌套截断提示缩小范围；③**错误可修复化**：按错误类
  （沙盒越界/不是目录/超时/SchemaError/参数错误/未检测到测试设施）追加
  "（建议：…）"。口径：**只在确有建议时加，不制造噪音**（有结果时不加）。
  顺带修正：S95 golden 契约测试把 `hint` 与错误尾注纳入"传输层装饰"剥离
  （fixture 捕获于该层之前，属实现装饰非 fs 语义）。
